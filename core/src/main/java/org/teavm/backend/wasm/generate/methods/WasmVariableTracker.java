...
class WasmVariableTracker {
    ...
    void storeToVariable(Variable variable) {
        var varDef = new VarDefinition(level, level.mostRecentVarDefinition, level.builder.list.getLast(), variable,
    ...
    void pushArgs(List<? extends Variable> args) {
        ...
        fillMissingArgs(args, 0, startArgsIndex);
        while (true) {
            var nextArgsIndex = findSuitableDefinition(args, endArgsIndex);
            if (nextArgsIndex < 0) {
                break;
            }
            fillMissingArgs(args, endArgsIndex, nextArgsIndex + 1);
            flushExtraArgs(args.get(endArgsIndex - 1), args.get(nextArgsIndex));
            endArgsIndex = nextArgsIndex + 1;
        }
        var entry = findEntry(args.get(startArgsIndex));
    ...
    private void fillMissingArgs(List<? extends Variable> args, int start, int end) {
        if (start + 1 == end) {
            return;
        }
        ...
        for (var i = start; i < end; ++i) {
            ...
            if (referenceInsn != null) {
                referenceInsn.insertNext(insn);
            } else {
                level.builder.list.addFirst(insn);
            }
            referenceInsn = insn;
        }
    }
    ...
    private static class VarDefinition {
        ...
        VarDefinition previous;
        WasmInstruction instruction;
        Variable variable;
        int order;
        boolean savedToLocal;

        VarDefinition(Level level, VarDefinition previous, WasmInstruction instruction, Variable variable, int order) {
            this.level = level;
